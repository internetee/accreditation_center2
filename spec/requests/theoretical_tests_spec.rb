...
RSpec.describe 'TheoreticalTests', type: :request do
  ...
  let!(:answer2) { create(:answer, question: question2, correct: true) }
  ...
  it 'starts a test and redirects to question if test attempt is found' do
    ...
    expect(test_attempt.question_responses.count).to eq(test.questions.active.count)
  end
  ...
  it 'serves a question and renders the question template if test attempt is completed and the previous question is not answered' do
    test_attempt.update(started_at: Time.current, completed_at: Time.current)
    get question_theoretical_test_path(test, attempt: test_attempt.access_code, question_index: 1)
  ...
  it 'rejects an answer and redirects to root if test attempt is not found' do
