class TestsController < ApplicationController
  ...
  def block_history_during_active_attempt!
    other_in_progress_attempt = current_user.test_attempts.in_progress.where.not(id: @test_attempt.id).exists?
    return unless other_in_progress_attempt && @test_attempt.in_progress?
