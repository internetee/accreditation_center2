...
# AccreditationCenter — AI Context

> Rails 8.1.3 | Ruby 3.4.4
> Generated by rails-ai-context v5.10.0

## Stack
- Database: static_parse — 12 tables
- Models: 12
- Routes: 123 across 20 controllers
- Async: 2 jobs
...
- **PracticalTask** (2a, 5v) — belongs_to :test, has_many :practical_task_results
  scopes: ordered, active
- **PracticalTaskResult** (2a, 2v) — belongs_to :test_attempt, belongs_to :practical_task
- **Question** (3a, 7v) — belongs_to :test_category, has_many :answers, has_many :question_responses
  scopes: ordered, active, mandatory, non_mandatory
- **QuestionResponse** (2a, 2v) — belongs_to :test_attempt, belongs_to :question
  scopes: answered
- **Registrar** (3a, 4v) — has_many :users, has_many :test_attempts, has_many :registrar_notification_events
  scopes: with_non_admin_users
- **RegistrarNotificationEvent** (1a, 3v) — belongs_to :registrar
- **Test** (5a, 8v) — has_many :test_categories_tests, has_many :test_categories, has_many :test_attempts, has_many :questions, has_many :practical_tasks
  scopes: active, auto_assignable
- **TestAttempt** (6a, 4v) — belongs_to :user, belongs_to :test, has_many :question_responses, has_many :questions, has_many :practical_task_results, has_many :practical_tasks
...
- **files**: activestorage
- **testing**: rspec-rails, minitest, factory_bot_rails, faker, capybara
...
**Services:** AccreditationResultsService, ApiConnector, ApiTokenService, AuthenticationService, BotAuthService, ContactService, DomainService, InvoiceService, OidcAuthenticationService, RegistrarAccreditationEligibility, RegistrarAccreditationNotificationsService, ReppDomainService
**Jobs:** AccreditationSyncJob, ExpiryCheckJob
...
- Follow hotwire + service_objects + stimulus + importmaps + concerns_models + concerns_controllers + validators + policies + pwa + docker + kamal + ci_github_actions + solid_queue + solid_cache + solid_cable + zeitwerk architecture
